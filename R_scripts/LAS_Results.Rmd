...
# Facet by Residency 
valid %>% 
ggplot(aes(x=Section1Q1, fill=Section1Q1)) +  
  geom_bar(aes(x = Section1Q1,
               y = after_stat(count/sum(count)))) +
...
# Prep the data frame
test <- tbl %>% 
  rename(Resident = `Resident Percent`, `Non-Resident` = `Non-Resident Percent`) %>% 
  filter(!Resident == " ",
         !` ` == "Total") %>% 
  pivot_longer(Resident:`Non-Resident`, names_to = "Residency", values_to = "PercentByResidency") %>% 
  mutate(PercentByResidency = as.numeric(PercentByResidency),
         PercentByResidency = PercentByResidency/100)

# Order 
test$` ` <- factor(test$` `, levels = c("Yes", "No", "Don't know/ Not Sure", "-99", NA))
test$Residency <- factor(test$Residency, levels = c("Non-Resident", "Resident"))


# Plot
ggplot(test, aes(fill=Residency, y=PercentByResidency, x=` `)) +
  geom_bar(position='dodge', stat='identity', alpha = 0.5) +
  
  geom_text(aes(label=(PercentByResidency*100)), position=position_dodge(.9), vjust=0) + 
